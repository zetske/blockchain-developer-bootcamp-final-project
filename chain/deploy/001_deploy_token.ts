...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  await deploy("MyNFT", {
    ...
    ], // tokenOwner is the address used as the first argument to the Token contract's constructor.
    log: true, // Display the address and gas used in the console (not when run in test though).
  });
};
export default func;
func.tags = ["MyNFT"]; // This sets up a tag so you can execute the script on its own (and its dependencies).
